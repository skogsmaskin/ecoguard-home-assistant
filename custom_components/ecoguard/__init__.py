...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _LOGGER.debug(
        "Setting up entry %s for domain %s", entry.entry_id, entry.data.get("domain")
    )
    ...
    try:
        ...
        async def _fetch_batch_data():
            ...
            try:
                ...
                _LOGGER.warning(
                    "Error in background data fetch: %s", err, exc_info=True
                )
        ...
        async def _fetch_latest_reception():
            ...
            try:
                ...
                if latest_reception:
                    latest_reception_coordinator.async_set_updated_data(
                        latest_reception
                    )
                    _LOGGER.debug(
                        "Latest reception data updated: %d entries",
                        len(latest_reception),
                    )
            except Exception as err:
                _LOGGER.warning(
                    "Error fetching latest reception: %s", err, exc_info=True
                )
        ...
        if hass.state == CoreState.running:
            ...
            _LOGGER.info(
                "Home Assistant already running, triggering data refresh after reload"
            )
            ...
            hass.bus.async_listen_once(
                "homeassistant_started", _start_data_fetching_after_startup
            )
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if unload_ok and hasattr(entry, "runtime_data"):
        ...
        if (
            runtime_data.entity_registry_update_task
            and not runtime_data.entity_registry_update_task.done()
        ):
            try:
            ...
            for cache_key, task in list(coordinator._pending_requests.items()):
                if not task.done():
                    try:
                        ...
                        # Task may already be cancelled or in an invalid state
                        _LOGGER.debug(
                            "Error cancelling pending request task for %s: %s",
                            cache_key,
                            e,
                        )
...
async def trigger_data_fetch_for_entry(hass: HomeAssistant, entry_id: str) -> None:
    ...
    async def _fetch_batch_data():
        ...
        try:
            ...
            _LOGGER.warning(
                "Error in data fetch for entry %s: %s", entry_id, err, exc_info=True
            )
    ...
    async def _fetch_latest_reception():
        ...
        try:
            ...
            if latest_reception:
                ...
                _LOGGER.debug(
                    "Latest reception data updated for entry %s: %d entries",
                    entry_id,
                    len(latest_reception),
                )
        except Exception as err:
            _LOGGER.warning(
                "Error fetching latest reception for entry %s: %s",
                entry_id,
                err,
                exc_info=True,
            )
