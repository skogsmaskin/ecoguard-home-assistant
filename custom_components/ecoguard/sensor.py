...
async def _async_get_translation(hass: HomeAssistant, key: str, **kwargs: Any) -> str:
    ...
    try:
        ...
        # Load translation file directly to access sensor section
        # (The translation helper only loads config section)
        translation_data = await _load_translation_file(hass, lang)

        if translation_data and "sensor" in translation_data:
            sensor_data = translation_data["sensor"]

            # Navigate the nested structure: sensor.utility.hw or sensor.name.daily_consumption
            key_parts = key.split(".")
            current = sensor_data
